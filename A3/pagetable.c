...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		if ((victim -> frame) & PG_DIRTY) {
			int off_set_out = swap_pageout(frame, victim -> swap_off);
			// ??????? Could check it
			if (off_set_out == INVALID_SWAP) {
				perror("Invalid Swap\n");
				exit(1);
			}
			victim -> swap_off = off_set_out;
			// After swap out, the frame is no longer valid
...
char *find_physpage(addr_t vaddr, char type) {
	...
	// If the page directory not valid, initialise one
	if ((pgdir[idx].pde & PG_VALID) == 0) {
		pgdir[idx] = init_second_level();
	}

	// Get the pointer to 2nd-level page table
	pgtbl_entry_t *second_ptable = (pgtbl_entry_t *)(pgdir[idx].pde & PAGE_MASK);

	// Use vaddr to get index into 2nd-level page table and initialize 'p'
	unsigned p_idx = PGTBL_INDEX(vaddr);
	p = &second_ptable[p_idx];
	...
	// The frame is invalid, 
		...
		// The frame is not on swap
		else {
			// Initialise the frame first
			p->frame = frame << PAGE_SHIFT;
