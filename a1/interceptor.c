...
void my_exit_group(int status)
{
	...
	pid_t remove_target = current -> pid;
	//lock
	spin_lock(&my_table_lock);
	del_pid(remove_target);
	//unlock
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	if (check_pid_monitored(reg.ax, pid)) {
...
int helper_intercept(pid_t pid, int syscall) {
...
int helper_release(int syscall) {
	if (current_uid() != 0) return -EPERM;
	//cannot de-intercept a syscall not been intercepted
...
int helper_start_monitoring(pid_t pid, int syscall) {
    //must be a valid pid
    if ((pid < 0 || pid_task(find_vpid(pid), PIDTYPE_PID) == NULL) && pid != 0) return -EINVAL;
    if (current_uid() != 0 && (pid == 0 || check_pids_same_owner(current->pid, pid) != 0)) return -EPERM;
    //if the caller is root
    if (pid == 0) {
    	//already been monitored by all pids
		if (table[syscall].monitored == 2) return -EBUSY;
		else{
			spin_lock(&my_table_lock);
			destroy_list(syscall);
    ...
    //if the caller is not root
    	...
    	if (table[syscall].monitored == 2) return -EBUSY;
        //if the pid has already been monitored
...
int helper_stop_monitoring(pid_t pid, int syscall) {
	//must be a valid pid
    if (pid < 0 || (pid_task(find_vpid(pid), PIDTYPE_PID) == NULL && pid != 0)) return -EINVAL;
    //check if the caller has the right permission
    if (current_uid() != 0 && (pid == 0 || check_pids_same_owner(current->pid, pid) != 0)) return -EPERM;
    //if this syscall has been monitored
    if (table[syscall].monitored == 0) return -EINVAL;
    ...
    if (pid == 0) {
        ...
        //if pid is not monitored by this pid
        if (check_pid_monitored(syscall, pid) == 0) return -EINVAL;
        //if all pids are monitored with the syscall
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	switch (cmd) {
		case REQUEST_SYSCALL_INTERCEPT:
			return helper_intercept(pid, syscall);

		case REQUEST_SYSCALL_RELEASE:
			return helper_release(syscall);

		case REQUEST_START_MONITORING:
			return helper_start_monitoring(pid, syscall);

		case REQUEST_STOP_MONITORING:
			return helper_stop_monitoring(pid, syscall);
