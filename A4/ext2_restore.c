...
int main(int argc, char **argv) {
  ...
  //check absolute path
	...
	unsigned int target_inode_num_reg = find_inode_num(argv[2],file_name,EXT2_FT_REG_FILE);
	unsigned int target_inode_num_slnk = find_inode_num(argv[2],file_name,EXT2_FT_SYMLINK);
	unsigned int target_inode_num_dir = find_inode_num(argv[2],file_name,EXT2_FT_DIR);
	...
	if (target_inode_num_reg != 0 || target_inode_num_slnk != 0){
		free(path);
		fprintf(stderr, "The target already exists\n");
		return -EEXIST;
	}
	...
	add_entry_restore(parent_inode,file_name);
	free(path);
